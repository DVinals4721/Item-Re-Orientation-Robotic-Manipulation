...
def plot_gait_steps(gait, num_steps_to_plot=None, plot_all_steps=False):
    ...
    plt.show()
class GaitGenerator:
    def __init__(self, dimensions, resolution, clearance_radius, num_arc_points,min_intersection_distance=0,arc_height_factor=1):
...
if __name__ == "__main__":
    ...
    # Create generator
    generator = GaitGenerator(dimensions, 0.5, 0.5, 4,1)

    # Generate paths
    generator.generate_paths(
        initial_centroid=np.array([0, 0, 0]),
        initial_orientation=np.array([0, 0, 0]),
        target_orientation=np.array([np.pi/4, np.pi/2, 0])
    )
